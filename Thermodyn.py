...
def bv_freq_dry(**kwargs):
    ...
    ''' creates layer field'''
    # layer = make_layer(meteo.agl_m,
    # depth_m=kwargs['depth_m'],centered=kwargs['centered'])
...
def bv_freq_moist(**kwargs):
    ...
    # ''' add fields  '''
    # if check_mixingr:
    #   th = theta2(K=meteo.K,hPa=meteo.pressure,
    # mixing_ratio=meteo.mixing_ratio)
    # else:
    #   th = theta1(K=meteo.K,hPa=meteo.pressure)
    ...
    if check_C and check_mixingr:
        ...
        d = {'temp': Tk, 'press': meteo.pressure,
             'sat_mixr': sat_mixr, 'layer': layer, 'theta': th}
    elif check_K and check_mixingr:
        th = theta2(
            K=meteo.K, hPa=meteo.pressure, mixing_ratio=meteo.mixing_ratio)
        sat_mixr = sat_mix_ratio(C=meteo.K-273.15, hPa=meteo.pressure)
        d = {'temp': meteo.K, 'press': meteo.pressure,
             'sat_mixr': sat_mixr, 'layer': layer, 'theta': th}
    elif check_C:
        Tk = meteo.C+273.15
...
def make_layer(height, **kwargs):
    ...
    layers = [
        layer_value[bisect.bisect_left(layer_value, item)] for item in height]
